...
function getTeamsByTournament(tournamentId) {
  ...
  const hasTeamPlayerId = hasColumn('team_players', 'player_id');
  const hasTeamSortOrder = hasColumn('team_players', 'sort_order');
  const selectPlayerId = hasTeamPlayerId ? 'tp.player_id' : 'NULL AS player_id';
  const selectSortOrder = hasTeamSortOrder ? 'tp.sort_order' : '1 AS sort_order';
  const playersJoin = hasTeamPlayerId ? 'LEFT JOIN players p ON p.id = tp.player_id' : 'LEFT JOIN players p ON 1=0';
  const playersOrder = hasTeamSortOrder ? 'tp.sort_order ASC,' : '';
  ...
  const links = db.prepare(
    `SELECT tp.id, tp.team_id, ${selectPlayerId}, tp.player_name, tp.handicap, ${selectSortOrder},
     ...
     ORDER BY tp.team_id ASC, ${playersOrder} tp.id ASC`
  ).all(tournamentId);
...
function getAvailablePlayersForTournament(tournamentId) {
  ...
  const teamPlayerJoin = hasTeamPlayerId ? 'tp.player_id = p.id' : '1=0';
  ...
  return db.prepare(
    `SELECT p.id, p.tournament_id, p.name, p.handicap, ${selectPlayerActive}, ${selectRyderCupSide}, ${selectCreatedAt}, ${selectUpdatedAt},
     ...
     FROM players p
     LEFT JOIN team_players tp ON ${teamPlayerJoin}
