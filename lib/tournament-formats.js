...
const LEGACY_FORMAT_MAP = {
  '2-manns scramble': 'scramble2',
  '2-mann scramble': 'scramble2',
  '2 man scramble': 'scramble2',
  '2-man scramble': 'scramble2',
  '3-manns scramble': 'scramble3',
  '3-mann scramble': 'scramble3',
  '3 man scramble': 'scramble3',
  '3-man scramble': 'scramble3',
  '4-manns scramble': 'scramble4',
  '4-mann scramble': 'scramble4',
  '4 man scramble': 'scramble4',
  '4-man scramble': 'scramble4',
  '2-ball bestball': 'bestball2',
  '2 ball bestball': 'bestball2',
  'scramble': 'scramble2',
  'texas scramble': 'scramble4',
  'slagspill': 'strokeplay',
  'stroke play': 'strokeplay',
  'enkeltformat': 'strokeplay',
  'single_format': 'strokeplay',
  'bestball': 'fourball',
  'four-ball': 'fourball',
  'scramble2': 'scramble2',
  'scramble3': 'scramble3',
...
function normalizeTournamentFormatLabel(format) {
  return normalizeTournamentFormat(format);
}

function getTournamentFormatConfig(format) {
  const normalizedFormat = normalizeTournamentFormat(format);
  const def = getFormatDefinition(normalizedFormat);
  return {
    key: normalizedFormat,
    label: def.label,
...
function getTeamSizeForFormat(format) {
  const size = getTournamentFormatConfig(format).teamSize;
  return size > 1 ? size : null;
}

function getTournamentFormatMeta(format) {
  const config = getTournamentFormatConfig(format);
  return {
    formatKey: config.definition.key,
    displayName: config.label,
    teamSize: config.teamSize,
    isTeamFormat: config.isTeamFormat
  };
}
